		...
		<el-container :class="['elContianer',isShowAside?'':'contain_box_in']">
			...
			<transition name = "select_in" >
				<el-aside width="320px"  v-show="isShowAside" class="select_box">
					...
					<el-row>
						<span class="span">业务价格条件</span>
						<div class="block">
							<el-select v-model="getKeyList.search_id" filterable placeholder="请选择" size="mini" class="date_box">
								<el-option v-for="item in priceList" :key="item.key" :label="item.value" :value="item.key"> </el-option>
							</el-select>
						</div>			
					</el-row>
					<el-row>
			...
			<el-main>
				...
				<el-row>
					<el-table  :data="tableData" :header-cell-style="{background:'#e0f4ff',color:'#000'}" border class="mainTable" size="mini" @selection-change="MainTableSelectChange" @row-click="mainTableTrClick">
						...
						<el-table-column align="center" :sortable="true" prop="business_price" label="业务价格条件字段"></el-table-column>
...
　　　　　　　　　pageSize: 10,
		...
		methods: {
			...
			//重置按钮
			reset(){
				this.getKeyList={page:1,per_page:10,search_id:"",status:"",user_id:"",segment_business_id:"",master_business_id:"",slaver_business_id:""}
				this.getMessage()
				this.sonTableIsShow=false;
			},
			//主业务类型change事件
			...
			getMessage(){
				var _this=this;
				this.$postFunc("/PriceMaintain/list",this.getKeyList,function(respones){
					_this.tableData=respones.data.data.result;
					_this.userlist=respones.data.data.user;
					_this.priceList=respones.data.data.pricelist;
			...
			getTRMessage(trid,name){
				var obj=new Object();
				obj.id=trid;
				obj.url="/PriceMaintain/priceshow/"+trid;
				obj.name='业务价格/业务板块类型关系 业务价格：'+name;
				obj.submitUrl="/PriceMaintain/detailsupdate/"+trid;
				this.sonTableIsShow=true;
				this.$refs.publicbussinessmodule.getTRMessage(obj)
			},
			//主表格tr点击展示次表格
			mainTableTrClick(row){
				this.getTRMessage(row.id,row.business_price)
			},
			//主表格选择框点击
            ...
            handleStart(){
                var _this=this;
				this.$batchEnable(this.MainTableSelectChangeIdList,"/PriceMaintain/statusUpdate",function(){
					_this.getMessage();
				})
            },
            //批量禁止
            handleProhibit(){
                var _this=this;
				this.$batchProhibit(this.MainTableSelectChangeIdList,"/PriceMaintain/statusUpdate",function(){
					_this.getMessage();
				})
            }
		}
