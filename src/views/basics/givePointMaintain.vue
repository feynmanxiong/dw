					...
					<div class="STableTitle clearfix">
					  ...
					  <template>
						<el-table :data="PortData" border size="mini" :header-cell-style="{background:'#e0f4ff',color:'#000'}">
							...
							<el-table-column align="center" prop="handle_man" label="操作人"></el-table-column>
...
　　　　　　　　　pageSize: 10,
		...
		methods: {
			...
			mainTableSingleDelete(id){
				...
				.then(() => {
					var _this=this;
					_this.$postFunc("/containerAddresses/destroy",{id:id},function(res){
			...
			getTRMessage(trid,name){
				var _this=this;
				_this.$postFunc("/containerAddresses/businessshow",{id:trid},function(res){
					console.log(res)
					...
					data.forEach(item=>{
						...
						obj.handle_man=item.handle_man;
				...
				this.$refs.buildSettlementCompany.validate(vail=>{
					if(vail){
						...
						if(this.innerVisibleType){
							//新增
							this.$postFunc("/containerAddresses/listadd",this.buildSettlementCompany,function(respones){
								_this.getMessage()
							...
							//修改
							this.$postFunc("/containerAddresses/listupdate",this.buildSettlementCompany,function(respones){
								_this.getMessage()
								_this.handleDialogClose();
							},function(){
							})
			...
			handleDelete() {
				if(this.MainTableSelectChangeIdList.length>0){
					...
					.then(() => {
						...
						var _this=this;
						_this.$postFunc("/containerAddresses/destroy",{id:ids},function(res){
							_this.getMessage();
						},function(){})
					}).catch(()=>{})
				}else{
			...
			handleDialogCommitSon(){
				...
				if(flag){
					...
					// dataBox.id=_this.chekcPortId;
					// dataBox.list=list;
					_this.$postFunc("/containerAddresses/detailsupdate/"+_this.chekcPortId,list,function(res){
            ...
            handleStart(){
                if(this.MainTableSelectChangeIdList.length>0){
					...
					.then(() => {
						...
						ids=ids.substring(0,ids.length-1);
						var _this=this;
						_this.$postFunc("/containerAddresses/statusupdate",{ids:ids,status:1},function(res){
							_this.getMessage();
						},function(){})
					}).catch(()=>{})
				}else{
            ...
            handleProhibit(){
                 if(this.MainTableSelectChangeIdList.length>0){
					...
					.then(() => {
						...
						ids=ids.substring(0,ids.length-1);
						var _this=this;
						_this.$postFunc("/containerAddresses/statusupdate",{ids:ids,status:0},function(res){
							_this.getMessage();
						},function(){})
					}).catch(()=>{})
				}else{
